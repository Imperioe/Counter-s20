...
public class FrameworkTests {
    ...
    public void test_createLocalGame(){
        ...
        LocalGame localGame = activity.createLocalGame(new CounterState(0));
    ...
    public void test_saveConfigButton(){
        ...
        ((Spinner) activity.tableRows.get(1).findViewById(R.id.playerTypeSpinner)).setSelection(1);
        gameConfig.removePlayer(1);
        gameConfig.addPlayer("Test",1);
        View view = activity.findViewById(R.id.saveConfigButton);
        activity.onClick(view);
        //TODO: Change to your game Main Activity
        CounterMainActivity reloadActivity = Robolectric.buildActivity(CounterMainActivity.class).create().resume().get();
    ...
    public void test_save_game() {
        ...
        counterState.setCounter(5);

        //Call save game (In Main)
        Saving.writeToFile(counterState, R.string.app_name + "_test", activity);

        //Test by remaking the state
        //TODO: Change to your game State
        CounterState newCounterState = new CounterState((CounterState) Saving.readFromFile(R.string.app_name + "_test", activity));

        //Check the game states are the same
        assertTrue(counterState.equals(newCounterState));
    }
